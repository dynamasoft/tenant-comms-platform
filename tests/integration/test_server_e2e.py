...
# This suite starts a real uvicorn server and makes live model + MCP calls. It is skipped by
# default so the standard `pytest` run stays fast and offline. Enable it explicitly with:
#   RUN_SERVER_E2E=1 uv run pytest tests/integration/test_server_e2e.py
pytestmark = pytest.mark.skipif(
    not os.getenv("RUN_SERVER_E2E"),
    reason="Live server e2e (real model + MCP). Set RUN_SERVER_E2E=1 to run.",
)
